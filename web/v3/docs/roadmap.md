...
3.  **[1.3] 三段式骨架**: 使用 `Splitter` 构建左-中-右基本布局。
    - **用户可见**: 看到典型的智能体工作台分栏结构，且可以手动拖拽调整宽度。

### 阶段二：导航与信息发现 (交付物：可交互的列表与标签页)
1.  **[2.1] 全局侧栏 (GlobalSidebar)**: 渲染左侧功能图标，并实现侧栏收缩。
    - **用户可见**: 侧栏图标排布美观，点击收缩/展开动画流畅。
2.  **[2.2] 组织标签页 (WorkspaceTabs)**: 实现胶囊风格的 Tab 切换。
    - **用户可见**: 顶部标签页可点击，点击时有选中动画反馈。
3.  **[2.3] 智能体列表 (AgentSidebar)**: 渲染模拟的智能体项（AgentItem）。
    - **用户可见**: 看到整齐的智能体列表，hover 有高亮，点击有左侧强调条。
