...
export class ConversationManager {
  ...
  /**
   * 从对话历史中移除指定的工具调用条目。
   * 工具调用存储在 assistant 消息的 tool_calls 数组中。
   * @param {string} agentId - 智能体ID
   * @param {string} toolCallId - 工具调用ID
   * @returns {{removed: boolean, messageRemoved: boolean, error?: string}} 
   *   - removed: 是否找到并移除了工具调用
   *   - messageRemoved: 是否移除了整个消息（当消息只包含该工具调用时）
   *   - error: 错误信息（如果有）
   */
  removeToolCallEntry(agentId, toolCallId) {
    const conv = this.conversations.get(agentId);
    
    if (!conv) {
      return { removed: false, messageRemoved: false, error: "conversation_not_found" };
    }

    if (!toolCallId || typeof toolCallId !== "string") {
      return { removed: false, messageRemoved: false, error: "invalid_tool_call_id" };
    }

    let removed = false;
    let messageRemoved = false;

    // 遍历对话历史，查找包含该工具调用的 assistant 消息
    for (let i = conv.length - 1; i >= 0; i--) {
      ...
      // 查找匹配的工具调用
      const toolCallIndex = msg.tool_calls.findIndex(call => call.id === toolCallId);
      
      if (toolCallIndex !== -1) {
        // 找到了工具调用
        removed = true;

        // 如果这是消息中唯一的工具调用，移除整个消息
        if (msg.tool_calls.length === 1) {
          conv.splice(i, 1);
          messageRemoved = true;
          
          if (this._logger) {
            void this._logger.debug?.("移除包含工具调用的整个消息", {
              agentId,
              toolCallId,
              messageIndex: i
            });
          ...
          // 否则只移除该工具调用
          msg.tool_calls.splice(toolCallIndex, 1);
          
          if (this._logger) {
            void this._logger.debug?.("从消息中移除工具调用", {
              agentId,
              toolCallId,
              messageIndex: i,
              remainingToolCalls: msg.tool_calls.length
            });
          }
        }
    ...
    if (!removed && this._logger) {
      void this._logger.warn?.("未找到要移除的工具调用", {
        agentId,
        toolCallId
      });
    }

    return { removed, messageRemoved };
  }

  /**
   * 从对话历史中移除指定工具调用的响应条目。
   * 工具响应存储为独立的消息，role 为 "tool"，包含 tool_call_id 字段。
   * @param {string} agentId - 智能体ID
   * @param {string} toolCallId - 工具调用ID
   * @returns {{removed: boolean, count: number, error?: string}}
   *   - removed: 是否找到并移除了至少一个响应
   *   - count: 移除的响应数量
   *   - error: 错误信息（如果有）
   */
  removeToolResponseEntry(agentId, toolCallId) {
    const conv = this.conversations.get(agentId);
    
    if (!conv) {
      return { removed: false, count: 0, error: "conversation_not_found" };
    }

    if (!toolCallId || typeof toolCallId !== "string") {
      return { removed: false, count: 0, error: "invalid_tool_call_id" };
    }
    ...
    for (let i = conv.length - 1; i >= 0; i--) {
      ...
      if (msg.tool_call_id === toolCallId) {
        ...
        if (this._logger) {
          void this._logger.debug?.("移除工具响应", {
            agentId,
            toolCallId,
            messageIndex: i
          });
    ...
    if (count === 0 && this._logger) {
      void this._logger.warn?.("未找到要移除的工具响应", {
        agentId,
        toolCallId
      });
    }
