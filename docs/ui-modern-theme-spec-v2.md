...
- N0 `#0B0F17`（页面底）
- N1 `#0F172A`（次级底）
- N2 `#151B26`（表面 1，深靛向中性偏移）
- N3 `#1E293B`（hover/选中弱底，中性深灰）
- N6 `#2A3A55`（分割线/描边）
- N8 `#6B7280`（弱信息，确保可读）
- N9 `#9CA3AF`（次文本，拉开层级）
- N10 `#EEF2F7`（主文本）
...
| `--surface-3` | `#F0F4F8` | hover/选中弱底（中性，温暖灰） |
| `--text-1` | `#0F172A` | 主文本 |
| `--text-2` | `#475569` | 次文本 |
| `--text-3` | `#8899AA` | 弱信息（时间戳、注释，避免发雾） |
| `--border` | `rgba(15,23,42,.10)` | 边框/分割线（更轻） |
| `--primary` | `#16B981` | 主色（偏青绿，更清新） |
| `--primary-weak` | `rgba(22,185,129,.18)` | 主色弱底（选中/气泡，增强可见度） |
| `--link` | `#0EA5E9` | 链接（向青色偏移，与主色和谐） |
| `--focus` | `rgba(22,185,129,.45)` | focus ring |
| `--danger` | `#DC2626` | 错误/删除（降低饱和度，更克制） |
| `--warning` | `#D97706` | 警告（降低饱和度） |
| `--info` | `#2563EB` | 信息（降低饱和度） |
| `--overlay` | `rgba(15,23,42,.30)` | 模态遮罩（减少压迫感） |

黑暗主题（Dark）
| Token | 值 | 用途 |
|---|---|---|
| `--bg` | `#0B0F17` | 页面背景（深蓝黑） |
| `--surface-1` | `#151B26` | 主表面（深靛，向中性偏移，避免过蓝） |
| `--surface-2` | `#0F172A` | 次表面 |
| `--surface-3` | `#1E293B` | hover/选中弱底（中性深灰） |
| `--text-1` | `#EEF2F7` | 主文本（更清晰） |
| `--text-2` | `#9CA3AF` | 次文本（降低亮度，拉开层级） |
| `--text-3` | `#6B7280` | 弱信息（避免发雾，确保可读） |
| `--border` | `rgba(238,242,247,.10)` | 边框/分割线 |
| `--primary` | `#2DD36F` | 主色（暗黑下更清晰但不荧光） |
| `--primary-weak` | `rgba(45,211,111,.20)` | 主色弱底（增强可见度） |
| `--link` | `#3B82F6` | 链接（降低亮度，减少刺眼） |
| `--focus` | `rgba(45,211,111,.50)` | focus ring |
| `--danger` | `#EF4444` | 错误/删除（降低亮度，更克制） |
| `--warning` | `#F59E0B` | 警告（降低亮度） |
| `--info` | `#3B82F6` | 信息（与链接统一，降低亮度） |
| `--overlay` | `rgba(0,0,0,.40)` | 模态遮罩（减少压迫感） |
...
### 4.8 可直接落地的语义 Token 清单（实现阶段守门规则）

本节提供"可直接落地"的语义 token 清单，用于实现阶段的自动化守门（禁止新增硬编码色）。所有组件必须且只能使用以下 token，不得引入任何硬编码颜色值。

#### 4.8.1 守门规则（实现阶段必须遵守）

**禁止行为：**
- 禁止在 CSS/SCSS/JSX 中直接使用 hex/rgb/rgba 颜色值（如 `#FFFFFF`、`rgb(0,0,0)`）
- 禁止引入"自定义灰"或"自定义绿"等非语义 token
- 禁止用彩色解决结构问题（结构只能由中性色体系负责）

**必须行为：**
- 所有颜色必须引用语义 token（如 `var(--bg)`、`var(--text-1)`）
- 每个组件的每个状态必须从本清单中选择对应的 token
- 新增组件必须先在清单中定义其 token 映射关系

**自动化检查建议：**
- 使用 ESLint/Stylelint 规则检测硬编码颜色
- 使用正则表达式检测 `#[0-9a-fA-F]{3,8}` 和 `rgb\(` 等硬编码模式
- CI 阶段强制执行 token 白名单检查

#### 4.8.2 全站必须实现的语义 Token 清单

以下 token 是全站最小集，任何页面都必须实现且只能使用这些 token：

| 分组 | Token 名称 | CSS 变量名 | 用途说明 | 禁止用法 |
|---|---|---|---|---|
| **结构层** | `bg` | `--bg` | 页面最外层背景 | 用彩色/状态色当结构底 |
| **结构层** | `surface-1` | `--surface-1` | 主内容表面（卡片、聊天、窗口内容） | 用透明叠层替代表面层级 |
| **结构层** | `surface-2` | `--surface-2` | 次表面（侧栏、分组底、header/footer） | 随意新增"自定义灰" |
| **结构层** | `surface-3` | `--surface-3` | 交互表面（hover、弱选中、分段底） | 用主色大面积铺底做 hover |
| **文本层** | `text-1` | `--text-1` | 正文/标题（主阅读） | 在浅底使用白字；在深底使用黑字 |
| **文本层** | `text-2` | `--text-2` | 次文本（元信息、说明） | 用透明白当次文本 |
| **文本层** | `text-3` | `--text-3` | 弱信息（时间戳、注释、placeholder） | 弱到"发雾"不可读 |
| **边界层** | `border` | `--border` | 分割线/描边（轻且清晰） | 用粗重边框当层级 |
| **聚焦层** | `focus` | `--focus` | focus ring（统一） | 用颜色堆砌焦点（多色/多重阴影） |
| **遮罩层** | `overlay` | `--overlay` | 模态遮罩 | 遮罩当背景层级使用 |
| **强调层** | `primary` | `--primary` | 主操作/选中标记/焦点主色 | 作为大面积结构底 |
| **强调层** | `primary-weak` | `--primary-weak` | 主色弱底（选中/气泡） | 用于大面积背景 |
| **交互层** | `selected` | `--selected` | 选中弱底（配合细强调） | 选中态用彩底白字承载长内容 |
| **交互层** | `link` | `--link` | 链接色（与主色分工） | 链接色与主色同强度抢焦点 |

**状态色（至少具备 danger；推荐四套齐全）：**

| 分组 | Token 名称 | CSS 变量名 | 用途说明 |
|---|---|---|---|
| **状态-危险** | `intent-danger-fg` | `--intent-danger-fg` | 危险前景（文字/图标） |
| **状态-危险** | `intent-danger-bg` | `--intent-danger-bg` | 危险弱底（背景，默认弱） |
| **状态-危险** | `intent-danger-border` | `--intent-danger-border` | 危险描边（细条/边框） |
| **状态-警告** | `intent-warning-fg` | `--intent-warning-fg` | 警告前景（文字/图标） |
| **状态-警告** | `intent-warning-bg` | `--intent-warning-bg` | 警告弱底（背景，默认弱） |
| **状态-警告** | `intent-warning-border` | `--intent-warning-border` | 警告描边（细条/边框） |
| **状态-信息** | `intent-info-fg` | `--intent-info-fg` | 信息前景（文字/图标） |
| **状态-信息** | `intent-info-bg` | `--intent-info-bg` | 信息弱底（背景，默认弱） |
| **状态-信息** | `intent-info-border` | `--intent-info-border` | 信息描边（细条/边框） |
| **状态-成功** | `intent-success-fg` | `--intent-success-fg` | 成功前景（文字/图标） |
| **状态-成功** | `intent-success-bg` | `--intent-success-bg` | 成功弱底（背景，默认弱） |
| **状态-成功** | `intent-success-border` | `--intent-success-border` | 成功描边（细条/边框） |

**可选 Token（仅用于小面积强强调场景）：**

| 分组 | Token 名称 | CSS 变量名 | 用途说明 |
|---|---|---|---|
| **强底-主色** | `on-primary` | `--on-primary` | 主色实心底上的文字/图标；不用于长段正文 |
| **强底-危险** | `on-danger` | `--on-danger` | 危险实心底上的文字/图标；不用于长段正文 |
| **强底-警告** | `on-warning` | `--on-warning` | 警告实心底上的文字/图标；不用于长段正文 |
| **强底-信息** | `on-info` | `--on-info` | 信息实心底上的文字/图标；不用于长段正文 |
| **强底-成功** | `on-success` | `--on-success` | 成功实心底上的文字/图标；不用于长段正文 |

#### 4.8.3 组件状态 Token 映射表（实现参考）

以下表格定义了每个组件在每个状态下应该使用的 token，实现时必须严格遵循：

##### 4.8.3.1 Button（按钮）

| Variant | State | 背景 Token | 文字/图标 Token | 边界 Token | 强调/补充 Token |
|---|---|---|---|---|---|
| Primary | default | `primary` | `on-primary` | `border`（可选） | 主操作仅此一类 |
| Primary | hover | `primary` | `on-primary` | `border`（可选） | 不改变文字色 |
| Primary | active | `primary` | `on-primary` | `border`（可选） | 允许轻下压 |
| Primary | focus-visible | `primary` | `on-primary` | `border`（可选） | `focus` ring |
| Primary | disabled | `surface-2` | `text-3` | `border` | 仍可读、不可点 |
| Secondary | default | `surface-1` | `text-1` | `border` | 次操作不抢戏 |
| Secondary | hover | `surface-3` | `text-1` | `border` | 只提升表面 |
| Secondary | active | `surface-3` | `text-1` | `border` | 允许轻下压 |
| Secondary | focus-visible | `surface-3` | `text-1` | `border` | `focus` ring |
| Secondary | disabled | `surface-2` | `text-3` | `border` | 仍可读、不可点 |
| Ghost/Icon | default | 透明 | `text-2` | 透明 | |
| Ghost/Icon | hover | `surface-3` | `text-1` | `border`（可选） | |
| Ghost/Icon | active | `surface-3` | `text-1` | `border`（可选） | 允许轻下压 |
| Ghost/Icon | focus-visible | `surface-3` | `text-1` | `border`（可选） | `focus` ring |
| Ghost/Icon | disabled | 透明 | `text-3` | 透明 | 仍可读、不可点 |
| Danger | default | `surface-1` | `intent-danger-fg` | `border` | 危险色优先用于文字/细条 |
| Danger | hover | `surface-3` | `intent-danger-fg` | `intent-danger-border` | 禁止大块红底吓人 |
| Danger | active | `surface-3` | `intent-danger-fg` | `intent-danger-border` | 允许轻下压 |
| Danger | focus-visible | `surface-3` | `intent-danger-fg` | `intent-danger-border` | `focus` ring |
| Danger | disabled | `surface-2` | `text-3` | `border` | 仍可读、不可点 |

##### 4.8.3.2 List Item（列表项：组织/智能体/插件）

| State | 背景 Token | 标题 Token | 次文本 Token | 分割线/边界 Token | 选中标记 Token |
|---|---|---|---|---|---|
| default | 透明或 `surface-2` | `text-1` | `text-3` | `border`（可选） | 无 |
| hover | `surface-3` | `text-1` | `text-2` | `border` | 无 |
| selected | `selected` | `text-1` | `text-2` | `border` | `primary` 细条（左侧/顶部） |
| disabled | `surface-2` | `text-2` | `text-3` | `border` | 无 |

**规则：**
- selected 的文本永远使用 `text-1`/`text-2`，禁止用 `on-primary` 或白字去"硬抬对比度"
- 细强调（左条/下划线）使用 `primary`，承担"定位"；弱底使用 `selected`，承担"归属"

##### 4.8.3.3 Tab（标签页）

| State | 背景 Token | 文字 Token | 边界 Token | 选中标记 Token |
|---|---|---|---|---|
| default | 透明 | `text-2` | 透明 | 无 |
| hover | `surface-3` | `text-1` | `border`（可选） | 无 |
| selected | `surface-1` | `text-1` | `border` | 下划线/细条使用 `primary` |
| disabled | 透明 | `text-3` | 透明 | 无 |

##### 4.8.3.4 Chat Bubble（聊天气泡）

| Role | 背景 Token | 文字 Token | 边界 Token | 备注 |
|---|---|---|---|---|
| user（我方） | `surface-1` 或 `selected` | `text-1` | `border` 或 `primary` 细条 | 禁止"绿底白字"做长段内容 |
| agent（对方） | `surface-1` | `text-1` | `border` | |

##### 4.8.3.5 Card（工具/思考/工件/附件）

| 区域 | 背景 Token | 标题 Token | 正文 Token | 边界 Token |
|---|---|---|---|---|
| 容器 | `surface-1` | `text-1` | `text-1` | `border` |
| Header | `surface-2` | `text-1` | `text-2` | `border`（下边） |
| Footer（可选） | `surface-2` | `text-2` | `text-2` | `border`（上边） |

##### 4.8.3.6 Modal / Window（模态与窗口）

| 部位 | 背景 Token | 文字 Token | 边界 Token | 备注 |
|---|---|---|---|---|
...
| close（hover） | `intent-danger-bg` | `on-danger` | `intent-danger-border` | 危险色只在 hover 出现 |

##### 4.8.3.7 Input（输入框）

| State | 背景 Token | 文字 Token | placeholder Token | 边界 Token | 备注 |
|---|---|---|---|---|---|
| default | `surface-1` | `text-1` | `text-3` | `border` | |
| hover | `surface-1` | `text-1` | `text-3` | `border` | |
| focus-visible | `surface-1` | `text-1` | `text-3` | `border` | `focus` ring |
| error | `surface-1` | `text-1` | `text-3` | `intent-danger-border` | 错误不铺红底 |
| disabled | `surface-2` | `text-3` | `text-3` | `border` | 仍可读、不可点 |

#### 4.8.4 Token 使用优先级（冲突时如何选择）

当同一个元素需要多个 token 时，按以下优先级选择：

1. **状态色优先于主色**：error 状态优先使用 `intent-danger-*`，而非 `primary`
2. **文本永远使用文本 token**：无论背景是什么颜色，文本必须使用 `text-1`/`text-2`/`text-3`
3. **结构永远使用中性色**：背景、表面必须使用 `bg`/`surface-*`，禁止用彩色做结构
4. **选中态固定为"弱底 + 细强调"**：背景用 `selected`，细条用 `primary`，文本用 `text-1`

#### 4.8.5 扩展 Token 的规则（当现有 token 不够用时）

如果遇到现有 token 无法满足的场景，按以下规则扩展：

1. **优先扩展语义层级**：例如需要更深的表面，新增 `surface-4`，而非引入"自定义灰"
2. **保持命名一致性**：新 token 必须遵循现有命名规范（如 `surface-*`、`text-*`）
3. **更新本清单**：新增 token 必须先在本清单中定义，才能在组件中使用
4. **禁止扩展色相**：不得新增 `custom-green`、`custom-blue` 等色相相关 token
