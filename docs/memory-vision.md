...
| **default_search_depth** | 默认搜索深度（跳数） | 2 |
| **max_search_results** | 单次搜索最大结果数 | 100 |
...
**搜索顺序**：
1. **关注点顺序**：从最新关注点到最老关注点依次扫描
2. **路线强度优先**：每个关注点的搜索按"路线强度"降序进行

**路线强度计算**：
- 路线强度 = 路径上所有关联强度的**乘积**
- 示例：A→B→C 的路线强度 = strength(A→B) × strength(B→C)

**搜索示例**：
```
关注点A（最新）
    │
    ├─► B (强度0.5) → E (强度1.0)  路线强度：0.5×1.0=0.5 (ABE)
    │
    ├─► C (强度0.6) → D (强度0.1)  路线强度：0.6×0.1=0.06 (ACD)
    │
    └─► B (强度0.5) → 直接到B      路线强度：0.5 (AB)

搜索顺序：AC(0.6) → AB(0.5) → ABE(0.5) → ACD(0.06)
```

**搜索规则**：
- 从最新关注点到最老关注点依次处理
- 每个关注点内部按路线强度降序遍历（优先队列）
- 在指定深度范围内扫描所有可达节点
- 关键词匹配则加入结果队列（去重处理）

**关注点为空时的处理**：
- 系统首次启动或清空后，关注点列表为空
- 此时 `recall()` 返回空结果（无记忆可回忆）
- 首次 `remember()` 后，新节点成为关注点，后续 `recall()` 正常工作
...
- 它们作为**搜索回忆的起始点**
- 随着智能体对话进行，新创建的记忆节点成为关注点
- 关注点随话题动态变化

**来源**：
当智能体调用 `remember(messages)` 时，从**新创建的记忆节点**中选择关注点：
```
语义切割后的记忆节点
    │
    ▼ 提取关键实体（名词、代词等）
...
**说明**：
- **不查找已有节点**：即使内容重复也创建新节点（副本独立性）
- 新创建的记忆节点本身成为关注点
- 关注点在**建立记忆时**确定，不是回忆时
- 随着对话进行，新节点加入，关注点动态更新
...
外部系统传入的聊天记录
    ...
    │
    ▼ 【步骤2】生成摘要和关键词
    │   小模型为每个segment生成phrase、keywords
    │
    ▼ 【步骤3】批量创建节点及同批次关联
...
**关联规则说明**：
- **同批次节点间**（步骤3）：强度0.5，正常衰减，关系名"上文"/"下文"
- **与当前关注点**（步骤4）：强度1.0（新建时），关系名由小模型生成
...
智能体对话产生聊天记录
    ...
    ▼ 【步骤2】生成摘要和关键词
    │
    ▼ 【步骤3】创建节点 + 同批次节点间关联（强度0.5，"上文"/"下文"）
    │
    ▼ 【步骤4】与当前关注点建立关联（强度1.0，语义关系名）
    │
    ▼ 【步骤5】新节点成为关注点（LRU维护）
    │
    ▼ 【步骤6】触发压缩
...
**问题**：任务队列可能因大量 remember 调用而无限增长

**处理机制**：
- 设置队列长度上限（如1000个任务）
- 队列满时，新任务入队失败，返回错误
- 调用方（外部系统）需要根据错误调整调用频率
...
**处理机制**：
- 使用 `visited` 集合（与循环检测共用）记录已返回的节点
- 同一节点只返回一次，避免结果重复
- 保留首次发现的路径信息
...
**处理机制**：
- 压缩时衰减关联强度后立即检查
- 强度 < link_break_threshold（0.01）时立即标记为断裂
- 断裂的关联不再参与后续搜索和重要性计算
...
- 新增：语义切割机制（8.2、8.3）
- **明确**：记忆建立5步流程（8.1、9.3）：语义切割→生成摘要→创建同批次关联→创建与关注点关联→更新关注点→触发压缩
- **修改**：新节点与关注点关联的关系名由小模型语义分析确定（8.1、8.3）
- **删除**：关注点之间不再自动建立特殊关联
- 新增：切割节点间关联强度=0.5规则（9.3）
- 新增：外部系统触发判断说明（8.1）
- 新增：边界情况处理（11.5节）：循环关联、队列深度限制、结果去重、关联断裂时机
- 新增：节点删除后关联懒删除机制（10.5）
- 新增：小模型压缩失败重试15次机制（10.5）
- **新增**：小模型语义切割失败处理机制（8.1、10.5）：失败时重新入队，下次重试
- 新增：扫描次数与长期记忆机制（2、9、10.2.4）
- 新增：扫描顺序优先处理次数少的节点（10.2.4）
- 新增：关注点机制（第8节）
- **澄清**：关注点本质是被标记的普通记忆节点，不查找已有节点
- 新增：关注点来源于聊天记录关键实体（8.3）
- 新增：关键词要求：名词/代词，指代事物/领域（7.3）
- **明确**：搜索策略为基于路径强度的最佳优先搜索：全局优先队列、路线强度乘积、depth为跳数（7.1）
- **新增**：搜索终止条件（遍历完或达到maxResults）和maxSearchResults参数（1节、7.1）
- **新增**：maxQueueSize参数（1节）
- **明确**：步骤3批量创建节点、步骤4关系名失败不写、步骤5最新focus_limit个节点成为关注点（8.1、9.3）
- **明确**：取消关注后关联从下次压缩开始衰减（8.4）
- **明确**：关注点→其他节点关联参与目标节点重要性计算（8.1、9.3）
- **明确**：LRU使用队列顺序（8.4）
- **新增**：悬空关联搜索时跳过（11.5）
- **新增**：明确关联强度不需要对称（5.1）
- 更新：回忆函数增加 `depth` 参数（6.1）
